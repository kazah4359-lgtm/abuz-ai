...
Если вы хотите запустить на этом сервере или локально ИИ-агента **Hermes Agent** (от Nous Research) для автоматизации, используйте подготовленный скрипт:

```bash
./scripts/install-hermes-agent.sh
```
После установки перезагрузите оболочку (`source ~/.bashrc`) и используйте команду `hermes` для настройки и общения с агентом.
Скрипт устанавливает агента в фоновом режиме, поэтому для начальной настройки вам необходимо будет выполнить команду `hermes setup`, а для запуска дашборда `hermes dashboard`.
